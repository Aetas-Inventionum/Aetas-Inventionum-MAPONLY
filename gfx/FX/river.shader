...
PixelShader = 
{
	...
	[[
		float4 main( VS_OUTPUT In ) : PDX_COLOR
		{
			...
			vBottom = GetOverlay( vBottom, ColorMap, 0.5f )*1.5; //added *2
			...
			float3 vColor = lerp( vBottom, vWaterSurface.xyz, vWaterSurface.a ) * 1.2; // changed from 0.8 to 0.2
			...
			float vFoW = GetFoW( In.vPrePos_Fade.xyz, vFoWColor, FoWDiffuse ) * 0.6; //added *0.5
